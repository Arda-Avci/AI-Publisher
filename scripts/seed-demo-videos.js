...
const crypto_js_1 = require("../src/lib/crypto.js");
...
async function generateBaseAssets() {
    const baseVideoPath = path_1.default.join(process.cwd(), 'videolar', 'demo_base.mp4');
    const uploadsDir = path_1.default.join(process.cwd(), 'uploads');
    const testMusicPath = path_1.default.join(uploadsDir, 'test_bg_music.mp3');
    await fs_extra_1.default.ensureDir(path_1.default.join(process.cwd(), 'videolar'));
    ...
    if (!await fs_extra_1.default.pathExists(baseVideoPath)) {
    ...
    if (!await fs_extra_1.default.pathExists(testMusicPath)) {
...
async function seed() {
    ...
    const defaultUsername = (0, crypto_js_1.encryptUsername)('arda.avci@gmail.com');
    let user = await db_js_1.db.get('SELECT * FROM users WHERE username = ?', [defaultUsername]);
    if (!user) {
        console.log('[WARN] arda.avci@gmail.com kullanıcısı bulunamadı, oluşturuluyor...');
        await db_js_1.db.run('INSERT INTO users (username, password, credits) VALUES (?, ?, ?)', [defaultUsername, 'adminpass', 10000]);
        user = await db_js_1.db.get('SELECT * FROM users WHERE username = ?', [defaultUsername]);
    }
    else {
        // Krediyi 10,000 yapalım
        await db_js_1.db.run('UPDATE users SET credits = 10000 WHERE id = ?', [user.id]);
    ...
    const oldJobs = await db_js_1.db.all("SELECT id, final_filename FROM video_jobs WHERE final_filename LIKE 'demo_video_%'");
    for (const j of oldJobs) {
        await db_js_1.db.run('DELETE FROM video_scenes WHERE job_id = ?', [j.id]);
        await db_js_1.db.run('DELETE FROM video_jobs WHERE id = ?', [j.id]);
        const fPath = path_1.default.join(process.cwd(), 'videolar', j.final_filename);
        if (await fs_extra_1.default.pathExists(fPath)) {
            await fs_extra_1.default.remove(fPath);
        }
        // Kapakları da temizle
        for (let i = 0; i < 3; i++) {
            const coverPath = path_1.default.join(process.cwd(), 'uploads', `cover_${j.id}_${i}.jpg`);
            if (await fs_extra_1.default.pathExists(coverPath))
                await fs_extra_1.default.remove(coverPath);
        }
    }
    ...
    for (let i = 0; i < demoScenarios.length; i++) {
        ...
        const destVideoPath = path_1.default.join(process.cwd(), 'videolar', finalFilename);
        // Ana video dosyasını kopyala
        await fs_extra_1.default.copy(baseVideoPath, destVideoPath, { overwrite: true });
        ...
        for (let cIdx = 0; cIdx < 3; cIdx++) {
            ...
            const cmd = `ffmpeg -y -f lavfi -i "color=c=${colors[cIdx]}:s=1280x720:d=1" -vframes 1 "${path_1.default.join(process.cwd(), 'uploads', `cover_demo_${index}_${cIdx}.jpg`)}"`;
            await new Promise((r) => (0, child_process_1.exec)(cmd, () => r()));
            coverPaths.push(`/uploads/cover_demo_${index}_${cIdx}.jpg`);
        }
        // video_jobs kaydı oluştur
        const jobRes = await db_js_1.db.run(`INSERT INTO video_jobs (
        ...
        production_template, background_music_path, tts_provider, cover_image_path, cover_images
            ...
            scenario.music,
            'xtts',
            path_1.default.join(process.cwd(), 'uploads', `cover_demo_${index}_0.jpg`),
        ...
        for (let sIdx = 0; sIdx < scenario.scenes.length; sIdx++) {
            ...
            await db_js_1.db.run(`INSERT INTO video_scenes (
