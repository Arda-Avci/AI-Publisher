...
async function main() {
    ...
    if (!portActive) {
        ...
        serverProcess = (0, child_process_1.spawn)('npx', ['tsx', 'src/server.ts'], {
    ...
    const browser = await playwright_1.chromium.launch({ headless: false, slowMo: 60 });
    ...
    try {
        ...
        if (iconBtns.length > 0) {
            ...
            await page.evaluate(() => window.openModal('opportunityModal'));
        }
        await delay(1000);
        ...
        if (modalVisible !== 'block') {
            ...
            await page.evaluate(() => {
                ...
                if (typeof window.openOppStep1 === 'function')
                    window.openOppStep1();
            });
            await delay(800);
        }
        ...
        if (searchInput) {
            ...
            await page.evaluate(() => {
                if (typeof window.addInterest === 'function')
                    window.addInterest('yapay zeka geleceği');
            });
            await delay(500);
            ...
            if (searchBtn) {
                const disabled = await searchBtn.evaluate((el) => el.disabled);
        ...
        if (ttsSelect) {
            ...
            await delay(500);
            const voiceVal2 = await page.$eval('#tts-voice-input', (el) => el.value);
            ...
            const allSelects = await page.$$eval('form select', els => els.map(e => e.name));
            console.log(`  ⚠️ TTS dropdown bulunamadı! Formdaki select'ler: [${allSelects.join(', ')}]`);
            const formHTML = await page.$eval('form[action="/create-job"]', (el) => el.innerHTML.substring(0, 500));
