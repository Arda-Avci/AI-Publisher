...
$models = @(
    ...
    @{Name="xtts"; Group="A"; Cmd='["/opt/conda/bin/python","-u","runpod_handler.py"]'; Extra='RUN apt-get install -y --no-install-recommends rubberband-cli rubberband-ladspa && rm -rf /var/lib/apt/lists/*\nRUN pip install coqui-tts edge-tts openai pyrubberband soundfile'; CopyRunpod=$true; Clone=$false},
    ...
    @{Name="musetalk"; Group="A"; Cmd='["/opt/conda/bin/python","-u","runpod_handler.py"]'; Extra='RUN pip install "transformers>=4.39.3" "accelerate==0.30.0" webdataset omegaconf safetensors einops av yacs gfpgan face-alignment facexlib\nRUN curl -sL "https://github.com/TMElyralab/MuseTalk/archive/main.tar.gz" | tar -xz -C /app && mv /app/MuseTalk-main /app/MuseTalk || curl -sL "https://github.com/TMElyralab/MuseTalk/archive/master.tar.gz" | tar -xz -C /app && mv /app/MuseTalk-master /app/MuseTalk'; CopyRunpod=$true; Clone=$false},
