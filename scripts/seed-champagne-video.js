...
async function main() {
    try {
        ...
        // İlk kullanıcıyı bul
        const user = await db_js_1.db.get('SELECT id, username FROM users LIMIT 1');
        if (!user) {
            logger_js_1.Logger.error('Veritabanında kayıtlı kullanıcı bulunamadı! Lütfen önce sunucuyu başlatıp varsayılan kullanıcının oluşmasını sağlayın.');
            process.exit(1);
        }
        logger_js_1.Logger.info(`Tohumlama yapılacak kullanıcı: ${user.username} (ID: ${user.id})`);
        // Şampanya video görevini ekle
        const result = await db_js_1.db.run(`
    ...
    `, [
        ...
        logger_js_1.Logger.info(`Şampanya video görevi başarıyla eklendi! Job ID: ${result.lastID}`);
        process.exit(0);
    }
    catch (err) {
        logger_js_1.Logger.error(`Tohumlama betiğinde hata: ${err instanceof Error ? err.stack : err}`);
        process.exit(1);
    }
}
main();
