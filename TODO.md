...
- [x] `colab_setup.py` ve `colab_server.py` dosyalarının her zaman git reposundaki en son dağıtımdan indirilmesi mantığının kurulması, `Google_Colab_AI_Publisher.ipynb` defterinin sıfırdan yazılarak git'e push edilmesi ve Colab entegrasyonunun tamamlanması.
