...
- [x] **Colab Docker Derleme Engellerinin Kaniko ve Yerel Registry ile Aşılması:** Colab VM'deki cgroup kısıtlamalarını aşmak için Kaniko (daemonless) ve hafif Go-tabanlı yerel Registry (localhost:5000) entegre edildi. Tüm betikler (`build_all.sh`, `patch_notebook.py` ve `colab_setup.py`) bu yönde güncellendi ve ilk hücre üzerinden de otomatik derleme kararlı hale getirildi. `service docker start` komutunun `docker: unrecognized service` hatası vermesi engellenerek `dockerd` doğrudan el ile arka planda başlatıldı. Kaniko binary indirme (wget 404) hatasını çözmek için binary doğrudan docker imajından kopyalandı.
