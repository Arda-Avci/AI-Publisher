...
- **Accelerate GradScaler İçe Aktarma Hatası (GradScaler ImportError)**: `accelerate` (v1.14.0) paketinin taklit edilen PyTorch sürümünü görerek `GradScaler` sınıfını eski konumu (`torch.cuda.amp`) yerine yeni konumu olan `torch.amp` altından içe aktarmaya çalışıp hata vermesi, `torch.amp` modülüne `GradScaler` nesnesi dinamik olarak enjekte edilerek çözüldü.
- **Generic Compiler Özelliği (torch.compiler.is_compiling AttributeError)**: `transformers` v5+ model kayıt süreçlerinde PyTorch >= 2.3 ile gelen `torch.compiler.is_compiling` ve `is_dynamo_compiling` fonksiyonlarının PyTorch 2.2.1'de bulunmamasından kaynaklanan `AttributeError` hatası, bu fonksiyonlar dummy `lambda: False` olarak `torch.compiler` modülüne yamalanarak çözüldü.
