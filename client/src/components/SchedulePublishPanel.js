...
export function SchedulePublishPanel({ language, t, onShowToast }) {
    ...
    const handleCreateSchedule = async (e) => {
        e.preventDefault();
        if (!selectedVideoId) {
            onShowToast?.(language === 'tr' ? 'Lütfen planlanacak bir video seçin.' : 'Please select a video to schedule.', 'error');
            return;
        }
        if (!scheduledDateTime) {
            onShowToast?.(language === 'tr' ? 'Lütfen bir yayınlama zamanı belirleyin.' : 'Please select a scheduled date and time.', 'error');
            return;
        }
        if (targetPlatforms.length === 0) {
            onShowToast?.(language === 'tr' ? 'En az bir platform seçmelisiniz.' : 'You must select at least one platform.', 'error');
            return;
        }
        ...
        try {
            ...
            if (r.ok && (d.success || d.schedule)) {
                onShowToast?.(language === 'tr' ? 'Paylaşım görevi başarıyla zamanlandı.' : 'Post successfully scheduled.', 'success');
    ...
    const handleDeleteSchedule = async (id) => {
        if (!confirm(language === 'tr' ? 'Bu paylaşım planını silmek istediğinize emin misiniz?' : 'Are you sure you want to delete this schedule?')) {
            return;
        }
        try {
    ...
    return (_jsxs("div", { style: {
            ...
            overflow: 'hidden'
                ...
                }, children: [_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '2px' }, children: [_jsxs("h3", { style: { margin: 0, fontSize: '14px', fontWeight: 600, color: '#A78BFA' }, children: ["\uD83D\uDCC5 ", t('schedule_title') || (language === 'tr' ? 'Sosyal Medya Yayın Planlayıcı' : 'Social Media Post Scheduler')] }), _jsx("p", { style: { margin: 0, fontSize: '11px', color: 'var(--text-muted)' }, children: language === 'tr'
                        ...
                        textAlign: 'left'
                                    ...
                                    color: '#9CA3AF'
                                        ...
                                        transition: 'background 0.2s'
                                                    ...
                                                    cursor: 'pointer',
                                                    transition: 'all 0.2s'
                                                }, onMouseEnter: e => e.currentTarget.style.background = 'rgba(239, 68, 68, 0.25)', onMouseLeave: e => e.currentTarget.style.background = 'rgba(239, 68, 68, 0.15)', children: language === 'tr' ? 'İptal Et' : 'Cancel' }) })] }, item.id))), schedules.length === 0 && !isLoading && (_jsx("tr", { children: _jsx("td", { colSpan: 5, style: { padding: '32px', textAlign: 'center', color: 'var(--text-muted)' }, children: language === 'tr' ? 'Planlanmış yayın bulunmamaktadır.' : 'No scheduled publications found.' }) }))] })] }) }), isModalOpen && (_jsx("div", { style: {
                    position: 'fixed',
                    inset: 0,
                        ...
                        boxShadow: '0 0 40px rgba(139, 92, 246, 0.2)'
                                                ...
                                                return (_jsxs("label", { style: {
                                                        ...
                                                        cursor: 'pointer'
                                                                ...
                                                                accentColor: '#8B5CF6',
                                                                cursor: 'pointer'
                                                            } }), _jsx("span", { style: { textTransform: 'uppercase', fontFamily: 'var(--font-mono)', fontSize: '10px' }, children: p })] }, p));
                                            }) })] }), _jsxs("div", { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '12px' }, children: [_jsx("button", { type: "button", onClick: () => setIsModalOpen(false), style: {
                                                padding: '8px 16px',
                                            ...
                                            }, children: isSubmitting ? (language === 'tr' ? 'Planlanıyor...' : 'Scheduling...') : (language === 'tr' ? 'Planla' : 'Schedule') })] })] })] }) }))] }));
}
