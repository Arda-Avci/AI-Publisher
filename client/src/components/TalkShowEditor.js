...
export function TalkShowEditor() {
    ...
    return (_jsxs("div", { style: s.panel, children: [_jsx("div", { style: { display: 'flex', gap: '8px' }, children: ['config', 'edit', 'progress'].map(m => (_jsxs("button", { onClick: () => setMode(m), style: {
                        ...
                        }, children: [scriptLoading ? _jsx(Loader, { size: 14, className: "pulse" }) : _jsx(Sparkles, { size: 14 }), scriptLoading ? 'Script Oluşturuluyor...' : 'AI ile Script Oluştur'] })] })), mode === 'edit' && (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '12px', flex: 1, overflow: 'hidden' }, children: [!selectedScript && (_jsxs("div", { style: s.card, children: [_jsxs("div", { style: s.sectionTitle, children: [_jsx(Edit3, { size: 14 }), " Script Se\u00E7"] }), scripts.length === 0 ? (_jsx("span", { style: { fontSize: '11px', color: 'var(--text-muted)' }, children: "Bu g\u00F6steri i\u00E7in script bulunamad\u0131. \u00D6nce script olu\u015Fturun." })) : (_jsx("div", { style: { display: 'flex', flexDirection: 'column', gap: '6px' }, children: scripts.map(s => (_jsxs("button", { onClick: () => selectScript(s.id), style: {
                                        textAlign: 'left', padding: '10px 12px', borderRadius: '6px',
                                        border: '1px solid var(--border)', background: 'rgba(255,255,255,0.02)',
                                        color: 'white', cursor: 'pointer', fontSize: '12px',
                                    }, children: [_jsx("div", { style: { fontWeight: 600 }, children: s.title }), _jsxs("div", { style: { fontSize: '10px', color: 'var(--text-muted)' }, children: [s.scene_count, " sahne \u00B7 ", s.status] })] }, s.id))) }))] })), selectedScript && (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px', flex: 1, overflow: 'auto' }, children: [_jsxs("div", { style: {
                                    ...s.card, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
                                }, children: [_jsx("div", { style: { fontSize: '13px', fontWeight: 'bold', color: 'white' }, children: selectedScript.title }), _jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [_jsxs("span", { style: { ...s.badge, background: 'var(--primary)', color: '#0b0f19' }, children: [selectedScript.segments.length, " Sahne"] }), _jsxs("button", { onClick: produceVideo, style: { ...s.btn, background: 'linear-gradient(135deg, #FF007F, #7F00FF)', color: 'white', fontSize: '11px' }, children: [_jsx(Play, { size: 12 }), " Video \u00DCret"] })] })] }), selectedScript.segments.map((seg, i) => (_jsxs("div", { style: {
                                    background: '#070a14', borderRadius: '8px', border: '1px solid var(--border)',
                                    padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px',
                                }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' }, children: [_jsxs("div", { style: { display: 'flex', gap: '8px', alignItems: 'center' }, children: [_jsx("span", { style: s.badge, children: i + 1 }), _jsx("span", { style: { ...s.badge, background: 'rgba(0,242,254,0.1)', color: 'var(--primary)', fontSize: '9px' }, children: seg.scene_type }), _jsx("span", { style: { fontSize: '12px', fontWeight: 600, color: 'var(--gold)' }, children: seg.character_name }), _jsxs("span", { style: { fontSize: '10px', color: 'var(--text-muted)' }, children: [seg.duration_seconds, "s \u00B7 ", seg.camera_instruction] })] }), _jsxs("div", { style: { display: 'flex', gap: '4px' }, children: [editingSegmentId !== seg.id ? (_jsx("button", { onClick: () => startEditing(seg), style: { background: 'none', border: 'none', color: 'var(--primary)', cursor: 'pointer', padding: '4px' }, children: _jsx(Edit3, { size: 12 }) })) : (_jsx("button", { onClick: () => saveSegment(seg.id), disabled: savingSegmentId === seg.id, style: { background: 'none', border: 'none', color: 'var(--success)', cursor: 'pointer', padding: '4px' }, children: savingSegmentId === seg.id ? _jsx(Loader, { size: 12, className: "pulse" }) : _jsx(Save, { size: 12 }) })), _jsx("button", { onClick: () => handleRegenerate(seg.id), disabled: regeneratingId === seg.id, style: { background: 'none', border: 'none', color: 'var(--secondary)', cursor: 'pointer', padding: '4px' }, children: regeneratingId === seg.id ? _jsx(Loader, { size: 12, className: "pulse" }) : _jsx(RefreshCw, { size: 12 }) })] })] }), (editingSegmentId === seg.id) ? (_jsxs("div", { style: { display: 'flex', gap: '6px' }, children: [_jsx("textarea", { value: editDialogue, onChange: e => setEditDialogue(e.target.value), style: { ...s.textarea, minHeight: '50px', fontSize: '11px', fontFamily: 'var(--font-mono)' } }), _jsx("button", { onClick: () => setEditingSegmentId(null), style: { background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '10px' }, children: _jsx(X, { size: 14 }) })] })) : (_jsx("div", { style: { fontSize: '11px', color: 'rgba(255,255,255,0.85)', lineHeight: '18px', fontFamily: 'var(--font-mono)' }, children: seg.dialogue_text }))] }, seg.id)))] }))] })), mode === 'progress' && (_jsx("div", { style: { ...s.card, flex: 1, justifyContent: 'center', alignItems: 'center', gap: '20px' }, children: _jsxs("div", { style: { textAlign: 'center', width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', gap: '16px' }, children: [_jsx("div", { style: { fontSize: '14px', fontWeight: 'bold', color: 'white' }, children: "Video \u00DCretim S\u00FCreci" }), _jsx("div", { style: s.progressBar, children: _jsx("div", { style: { ...s.progressFill, width: `${progressPercent}%` } }) }), _jsxs("div", { style: { fontSize: '24px', fontWeight: 'bold', color: 'var(--primary)' }, children: ["%", progressPercent] }), _jsxs("div", { style: { fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }, children: [jobStatus !== 'completed' && _jsx(Loader, { size: 14, className: "pulse" }), progressMsg || 'İşleniyor...'] }), jobStatus === 'completed' && (_jsxs("div", { style: { display: 'flex', gap: '10px', justifyContent: 'center' }, children: [_jsx("span", { style: { ...s.badge, background: 'var(--success)', color: 'white' }, children: "\u2713 Video Haz\u0131r" }), _jsx("button", { onClick: () => setMode('config'), style: { ...s.btn, background: 'var(--bg-surface)', color: 'white', border: '1px solid var(--border)', fontSize: '11px' }, children: "Yeni Proje" })] }))] }) }))] }));
}
