...
export function AiAssistantPanel({ language: _language, t: _t }: AiAssistantPanelProps) {
  ...
  const handleApplyTheme = () => {
    ...
    Object.entries(colors).forEach(([key, value]) => {
      document.documentElement.style.setProperty(`--${key.replace(/([A-Z])/g, '-$1').toLowerCase()}`, value as string);
    });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', color: 'white' }}>
      {/* Sub Tabs Header */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '800px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                Platformdaki herhangi bir özelliğin nasıl kullanılacağını gösteren, viral dikey formatta Shorts / TikTok senaryo ve görsel prompt planını oluşturun.
              </p>
            </div>
                    ...
                    <div key={scene.sceneNumber} style={{
                      ...
                      <div className="glass" style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--accent)', fontFamily: 'var(--font-mono)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)' }}>GÖRSEL PROMPT</span>
                            <button
                              onClick={() => handleCopy(scene.videoPrompt, `v-${scene.sceneNumber}`)}
                              style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}
                            >
                              {copiedId === `v-${scene.sceneNumber}` ? <Check size={14} style={{ color: 'var(--success)' }} /> : <Copy size={14} />}
                            </button>
                          </div>
                          <p style={{ fontSize: '13px', margin: 0, color: 'white', lineHeight: '20px', background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.02)' }}>
                            {scene.videoPrompt}
                          </p>
                        </div>
                        ...
                        {scene.speechText && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <span style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)' }}>SESLENDİRME METNİ</span>
                              <button
                                onClick={() => handleCopy(scene.speechText, `s-${scene.sceneNumber}`)}
                                style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}
                              >
                                {copiedId === `s-${scene.sceneNumber}` ? <Check size={14} style={{ color: 'var(--success)' }} /> : <Copy size={14} />}
                              </button>
                            </div>
                            <p style={{ fontSize: '13px', margin: 0, color: 'var(--text-muted)', lineHeight: '20px', fontStyle: 'italic' }}>
                              "{scene.speechText}"
                            </p>
                          </div>
                        )}
        ...
        {activeSubTab === 'landing' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '850px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <h3 style={{ fontSize: '18px', fontWeight: 600 }}>Açılış Sayfası Varlık Tasarımcısı</h3>
              <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                Landing page galerisi veya vitrin için konsept / niş bazlı video ve kapak görseli üretim promptlarını tasarlayın.
              </p>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '28px', marginTop: '10px' }}>
                ...
                <div className="glass" style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '14px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Açılış Sayfası Hero Tanıtım Videosu</span>
                  </div>
                  <div>
                    <h4 style={{ fontSize: '16px', fontWeight: 700, margin: '0 0 6px 0' }}>{landingResult.heroVideo.title}</h4>
                    <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: 0 }}>{landingResult.heroVideo.description}</p>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', background: 'rgba(0,0,0,0.3)', padding: '12px', borderRadius: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 'bold' }}>HERO VIDEO PROMPT</span>
                      <button
                        onClick={() => handleCopy(landingResult.heroVideo.prompt, 'hero-p')}
                        style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}
                      >
                        {copiedId === 'hero-p' ? <Check size={14} style={{ color: 'var(--success)' }} /> : <Copy size={14} />}
                      </button>
                    </div>
                    <p style={{ fontSize: '13px', margin: 0, lineHeight: '20px', color: 'white' }}>{landingResult.heroVideo.prompt}</p>
                  </div>
                </div>

                {/* Showcase Showcase Videos */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  <h4 style={{ fontSize: '14px', fontWeight: 'bold', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    {landingResult.showcaseVideos.map((item: any, idx: number) => (
                      <div key={idx} className="glass" style={{
                        ...
                        <div>
                          <span style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '4px', background: 'var(--accent-light)', color: 'var(--accent)', fontWeight: 'bold' }}>
                            {item.category.toUpperCase()}
                          </span>
                          <h5 style={{ fontSize: '14px', fontWeight: 700, margin: '8px 0 4px 0' }}>{item.title}</h5>
                          <p style={{ fontSize: '11px', color: 'var(--text-muted)', margin: 0 }}>{item.description}</p>
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', background: 'rgba(0,0,0,0.15)', padding: '10px', borderRadius: '6px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '9px', color: 'var(--text-muted)', fontWeight: 'bold' }}>VİDEO PROMPT</span>
                            <button
                              onClick={() => handleCopy(item.videoPrompt, `sc-v-${idx}`)}
                              style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}
                            >
                              {copiedId === `sc-v-${idx}` ? <Check size={12} style={{ color: 'var(--success)' }} /> : <Copy size={12} />}
                            </button>
                          </div>
                          <p style={{ fontSize: '12px', margin: 0, color: 'white', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
                            {item.videoPrompt}
                          </p>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', background: 'rgba(0,0,0,0.15)', padding: '10px', borderRadius: '6px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '9px', color: 'var(--text-muted)', fontWeight: 'bold' }}>KAPAK GÖRSELİ PROMPT</span>
                            <button
                              onClick={() => handleCopy(item.coverPrompt, `sc-c-${idx}`)}
                              style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}
                            >
                              {copiedId === `sc-c-${idx}` ? <Check size={12} style={{ color: 'var(--success)' }} /> : <Copy size={12} />}
                            </button>
                          </div>
                          <p style={{ fontSize: '12px', margin: 0, color: 'white', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
                            {item.coverPrompt}
        ...
        {activeSubTab === 'theme' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '800px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                Yapay zekadan istediğiniz atmosferi tanımlayarak yepyeni bir arayüz stili üretmesini isteyin. Üretilen renk paletini tek tıkla canlı olarak uygulayın!
              </p>
            </div>
            ...
            {themeResult && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '24px', marginTop: '10px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  <h4 style={{ fontSize: '15px', fontWeight: 700 }}>{themeResult.themeName} Renkleri</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                    {Object.entries(themeResult.colors).map(([name, val]: [string, any]) => (
                      <div key={name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '12px' }}>
                        <span style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>{name}</span>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px' }}>{val}</span>
                          <span style={{
                            width: '14px',
                            height: '14px',
                            borderRadius: '3px',
                  ...
                  <button
                    ...
                    className="btn btn-primary"
                    style={{ padding: '12px', gap: '8px', width: '100%', fontSize: '13px', fontWeight: 'bold' }}
                  >
                    <Palette size={16} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <div style={{
                    ...
                    {/* Header preview */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid hsl(${themeResult.colors.border})`, paddingBottom: '8px' }}>
                      <span style={{ fontSize: '13px', fontWeight: 'bold' }}>{themeResult.themeName} Studio</span>
                      <span style={{ fontSize: '10px', color: `hsl(${themeResult.colors.mutedForeground})` }}>Kredi: 1,500</span>
                    </div>
                    ...
                    <div style={{
                      ...
                      <span style={{ fontSize: '11px', fontWeight: 'bold', color: `hsl(${themeResult.colors.accent})` }}>VİDEO PROJESİ #21</span>
                      <p style={{ fontSize: '12px', margin: 0, color: `hsl(${themeResult.colors.foreground})` }}>
