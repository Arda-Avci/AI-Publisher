...
const s = {
  ...
  sidebar: {
    ...
    padding: '12px 0', overflowY: 'auto' as const, flexShrink: 0,
  },
...
const navItemStyle = (active: boolean): React.CSSProperties => ({
  ...
  fontSize: 13, textAlign: 'left' as const, transition: 'all 0.2s',
  borderRight: active ? '2px solid var(--accent)' : '2px solid transparent',
});

const themeCardStyle = (active: boolean): React.CSSProperties => ({
  padding: 8, borderRadius: 8, cursor: 'pointer', textAlign: 'center' as const,
...
const toggleStyle = (on: boolean): React.CSSProperties => ({
  ...
  position: 'relative' as const, transition: 'all 0.2s', padding: 0,
});

const toggleKnobStyle = (on: boolean): React.CSSProperties => ({
  width: 18, height: 18, borderRadius: '50%', background: 'white',
  position: 'absolute' as const, top: 2, left: on ? 20 : 2, transition: 'all 0.2s',
});
...
export function SettingsModal({ isOpen, onClose, language, theme, isDark, csrfToken, onSetTheme, onToggleDark, onToggleLanguage, t }: SettingsModalProps) {
  ...
  const createCharacter = async () => {
    ...
    try {
      ...
      setNewCharName(''); setNewCharDesc(''); setNewCharAvatar('');
      loadCharacters();
  ...
  const encodeFileAsBase64 = (file: File): Promise<string> => new Promise((resolve, reject) => {
  ...
  const tabs: { id: SettingsTab; label: string; icon: React.ReactNode }[] = [
    { id: 'appearance', label: t('settingsAppearanceTab') || 'Görünüm', icon: <Palette size={16} /> },
    ...
    <div style={s.overlay} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={s.modal}>
        ...
        <div style={s.body}>
          <div style={s.sidebar}>
            {tabs.map(tab => (
              <button key={tab.id} style={navItemStyle(activeTab === tab.id)} onClick={() => setActiveTab(tab.id)}>
                {tab.icon} {tab.label}
              </button>
            ))}
          </div>
            ...
            {activeTab === 'appearance' && (
              <div>
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>{t('pickapremiumcol109') || 'Premium bir tema seçin'}</div>
                  <div style={s.themeGrid}>
                    {THEMES.map(th => (
                      <div key={th.id} style={themeCardStyle(theme === th.id)}
                        onClick={() => { onSetTheme(th.id); fetch('/save-settings', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-csrf-token': csrfToken }, body: JSON.stringify({ selected_theme: th.id }) }).catch(() => {}); }}>
                        <div style={{ height: 40, borderRadius: 4, background: th.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 6, position: 'relative', overflow: 'hidden' }}>
                          <div style={{ width: 10, height: 10, borderRadius: '50%', background: th.color, boxShadow: `0 0 8px ${th.color}` }} />
                        </div>
                ...
                <div style={s.section}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={() => { if (!isDark) { onToggleDark(); fetch('/save-settings', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-csrf-token': csrfToken }, body: JSON.stringify({ theme_mode: 'dark' }) }).catch(() => {}); } }} style={{
                      ...
                      background: isDark ? 'var(--accent)' : 'var(--bg-surface)',
                      color: isDark ? 'white' : 'var(--text-muted)',
                    }}>🌙 {t('dark') || 'Karanlık'}</button>
                  </div>
                </div>
                <div style={s.section}>
                  <div style={s.sectionTitle}>{t('themetransition113') || 'Tema Geçişi'}</div>
                  <div style={s.sectionDesc}>{t('smoothtransitio114') || 'Yumuşak tema geçişleri'}</div>
                  <label style={s.toggleRow}>
                    <span style={s.toggleLabel}>{t('enableanimation115') || 'Animasyonları Etkinleştir'}</span>
            ...
            {activeTab === 'language' && (
              <div>
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>{t('chooseyourprefe116') || 'Tercih ettiğiniz dili seçin'}</div>
                  <div style={langCardStyle(language === 'tr')} onClick={() => { if (language !== 'tr') { onToggleLanguage(); fetch('/api/v1/set-language', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lang: 'tr' }) }).catch(() => {}); }}}>
                    <span style={{ fontSize: 24 }}>🇹🇷</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: 14 }}>Türkçe</div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{t('turkishinterfac117') || 'Türkçe arayüz'}</div>
                    </div>
                    {language === 'tr' && <span style={{ color: 'var(--accent)', fontSize: 18 }}>✓</span>}
                  </div>
                  <div style={langCardStyle(language === 'en')} onClick={() => { if (language !== 'en') { onToggleLanguage(); fetch('/api/v1/set-language', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lang: 'en' }) }).catch(() => {}); }}}>
                    <span style={{ fontSize: 24 }}>🇬🇧</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: 14 }}>English</div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{t('englishinterfac118') || 'English interface'}</div>
                    </div>
                    {language === 'en' && <span style={{ color: 'var(--accent)', fontSize: 18 }}>✓</span>}
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'account' && (
              <div>
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>{t('uploadyourprofi119') || 'Profil resminizi yükleyin'}</div>
                  <input ref={fileInputRef} type="file" accept="image/*" style={{ display: 'none' }}
                    onChange={async (e) => { const f = e.target.files?.[0]; if (f) { const b64 = await encodeFileAsBase64(f); await fetch('/save-settings', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-csrf-token': csrfToken }, body: JSON.stringify({ personal_avatar_base64: b64 }) }); } }} />
                  <button onClick={() => fileInputRef.current?.click()} style={{ ...s.btnStatic, background: 'var(--bg-surface)', color: 'var(--text-primary)', border: '1px solid var(--border)' }}>
            ...
            {activeTab === 'production' && (
              <div>
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>{t('textpositioning120') || 'Video üzerindeki metin konumu'}</div>
                  <select style={s.select} value={textGrid} onChange={e => setTextGrid(e.target.value)}>
                ...
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>{t('defaultnarrator121') || 'Varsayılan anlatıcı tonu'}</div>
                  <input style={s.input} value={narratorTone} onChange={e => setNarratorTone(e.target.value)}
                    placeholder={t('defaultNarratorPlaceholder') || 'Örn: Profesyonel, samimi, dramatik'} />
                </div>
                <div style={s.section}>
                  <div style={s.sectionTitle}>YouTube API Key</div>
                  <div style={s.sectionDesc}>{t('apikeyforyoutub122') || 'YouTube Data API anahtarı'}</div>
                  <input style={{ ...s.input, fontFamily: 'var(--font-mono)' }} value={ytApiKey} onChange={e => setYtApiKey(e.target.value)} placeholder="AIzaSy..." />
                </div>
                <div style={s.section}>
                  <div style={s.sectionTitle}>{t('wav2liplipsync123') || 'Wav2Lip Senkronizasyonu'}</div>
                  <div style={s.sectionDesc}>{t('reallipsyncviaw124') || 'Gerçek dudak senkronizasyonu'}</div>
                  <label style={s.toggleRow}>
                    <span style={s.toggleLabel}>{t('enablelipsync125') || 'Dudak senkronizasyonu'}</span>
                    <button style={toggleStyle(applyLipsync)} onClick={() => setApplyLipsync(!applyLipsync)}>
                ...
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>{t('addsavatarwatch127') || 'Avatar ve kanal abone ekranı'}</div>
                  <label style={s.toggleRow}>
                    <span style={s.toggleLabel}>{t('enableendscreen128') || 'Bitiş ekranı'}</span>
                    <button style={toggleStyle(applyEndScreen)} onClick={() => setApplyEndScreen(!applyEndScreen)}>
                ...
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>Videolarda kullanılacak marka logosu, renkler ve yazı tipi</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                      <label style={s.label}>Marka Logosu</label>
                      <input ref={brandLogoInputRef} type="file" accept="image/*" style={{ display: 'none' }}
                        onChange={async (e) => { const f = e.target.files?.[0]; if (f) setBrandLogo(await encodeFileAsBase64(f)); }} />
                      <button onClick={() => brandLogoInputRef.current?.click()} style={{ ...s.btnStatic, background: 'var(--bg-surface)', color: 'var(--text-primary)', border: '1px solid var(--border)' }}>
                        <Upload size={14} /> Yükle
                      </button>
                      {brandLogo && <div style={{ marginTop: 4, fontSize: 11, color: 'var(--success)' }}>✓ Yüklendi</div>}
                    </div>
                    <div>
                      ...
                      <input type="color" value={brandPrimaryColor} onChange={e => setBrandPrimaryColor(e.target.value)}
                        style={{ width: '100%', height: 36, border: '1px solid var(--border)', borderRadius: 6, background: 'var(--bg-surface)', cursor: 'pointer', padding: 2 }} />
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
                    <div>
                      <label style={s.label}>İkincil Renk</label>
                      <input type="color" value={brandSecondaryColor} onChange={e => setBrandSecondaryColor(e.target.value)}
                        style={{ width: '100%', height: 36, border: '1px solid var(--border)', borderRadius: 6, background: 'var(--bg-surface)', cursor: 'pointer', padding: 2 }} />
                    </div>
                    <div>
                      <label style={s.label}>Yazı Tipi</label>
                      <input style={s.input} value={brandFontPath} onChange={e => setBrandFontPath(e.target.value)} placeholder="Arial" />
                    </div>
                  </div>
                </div>
                <div style={s.section}>
                  <div style={s.sectionTitle}>Ses Klonlama (Voice Cloning)</div>
                  <div style={s.sectionDesc}>Kendi sesinizi klonlamak için kısa bir ses kaydı yükleyin</div>
                  <input ref={voiceInputRef} type="file" accept="audio/*" style={{ display: 'none' }}
                    onChange={async (e) => { const f = e.target.files?.[0]; if (f) setPersonalVoice(await encodeFileAsBase64(f)); }} />
                  <button onClick={() => voiceInputRef.current?.click()} style={{ ...s.btnStatic, background: 'var(--bg-surface)', color: 'var(--text-primary)', border: '1px solid var(--border)' }}>
                    <Upload size={14} /> Ses Dosyası Yükle
                  </button>
                  {personalVoice && <div style={{ marginTop: 4, fontSize: 11, color: 'var(--success)' }}>✓ Ses dosyası yüklendi</div>}
                </div>
                <button onClick={saveSettings} disabled={saving} style={{ ...s.btnStatic, background: 'var(--accent)', color: 'white', marginTop: 16 }}>
            ...
            {activeTab === 'characters' && (
              <div>
                <div style={s.section}>
                  ...
                  <div style={s.sectionDesc}>Videolarınızda @karakter_adı ile çağırabileceğiniz karakterler</div>
                ...
                <div style={s.section}>
                  ...
                  {characters.length === 0 ? (
                    <div style={{ fontSize: 13, color: 'var(--text-muted)', fontStyle: 'italic' }}>Henüz karakter eklenmemiş.</div>
                  ) : (
                    characters.map(c => (
                      <div key={c.id} style={s.charCard}>
                        <div style={s.charAvatar}>
                          {c.reference_image_base64
                            ? <img src={c.reference_image_base64} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            : <User size={18} style={{ color: 'var(--accent)' }} />}
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 600, fontSize: 13 }}>@{c.name}</div>
                          <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{c.description || 'Açıklama yok'}</div>
                        </div>
                        <button onClick={() => deleteCharacter(c.id)} style={{ ...s.btnStatic, background: 'rgba(239,68,68,0.1)', color: 'var(--danger)' }}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    ))
