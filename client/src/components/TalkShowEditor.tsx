...
export function TalkShowEditor() {
  ...
  const handleRegenerate = async (segmentId: number) => {
    ...
    try {
      const r = await fetch(
        `/api/v1/talkshow/scripts/${selectedScript.id}/regenerate/${segmentId}`,
        {
          method: 'POST',
          credentials: 'include',
        },
  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (d.stageKey) setProgressMsg(d.dockerMessage || d.stageKey);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={s.card}>
            <div style={s.sectionTitle}>
              <Monitor size={14} /> Gösteri Seç / Oluştur
            </div>
            <div>
              ...
              >
                ...
                {shows.map((s: any) => (
                  <option key={s.id} value={s.id}>
                    {s.master_prompt?.substring(0, 50)} (#{s.id})
                  </option>
                ))}
              </select>
            </div>
          ...
          <div style={s.card}>
            <div style={s.sectionTitle}>
              <Monitor size={14} /> Hedef Platformlar
            </div>
      ...
      {mode === 'edit' && (
        ...
        >
          ...
          {!selectedScript && (
            <div style={s.card}>
              <div style={s.sectionTitle}>
                <Edit3 size={14} /> Script Seç
              </div>
              {scripts.length === 0 ? (
                <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                  Bu gösteri için script bulunamadı. Önce script oluşturun.
                </span>
              ) : (
