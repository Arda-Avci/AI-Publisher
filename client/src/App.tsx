...
} from './components/CharacterSelectorModal.js';
...
export default function App() {
  ...
  const fetchSession = async () => {
    try {
      ...
      if (d2.userId) {
        ...
        if (d2.theme) setTheme(d2.theme);
        if (d2.lang) setLanguage(d2.lang);
        if (d2.isDark !== undefined) setIsDark(d2.isDark);
      } else {
        setIsLoggedIn(false);
        setIsAdmin(false);
      }
  ...
  const fetchCredits = async () => {
    try {
      ...
      const d = await r.json();
      if (d.success) setUserCredits({ credits: d.credits, limit: d.limit, resetDate: d.resetDate });
    } catch {}
  };
  const fetchJobs = async () => {
    try {
      ...
      else if (d.success && Array.isArray(d.jobs)) setJobs(d.jobs);
    } catch {}
  };
  ...
  const handleAddScene = async () => {
    ...
    try {
      ...
      if ((await r.json()).success) fetchScenes(selectedJob.id);
    } catch {}
  };
  ...
  const handleUseAsPrompt = (video: OpportunityVideo) => {
    ...
    setProductionNotes(
      `YouTube Link: https://youtube.com/watch?v=${video.videoId}\n${video.description}`,
    );
    setActiveTab('create');
  };
  ...
  const handleCharModalConfirm = async (
    charMap: Record<string, { name: string; description: string; isNew: boolean }>,
  ) => {
              ...
              <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                {(mainTab === 'Stüdyo' || mainTab === 'Galeri') && (
                  ...
                  >
                    <ProjectForm
                      ...
                      kineticSubtitlesStyle={kineticSubtitlesStyle}
                      onSetKineticSubtitlesStyle={setKineticSubtitlesStyle}
                      ...
                      onSetCamIntensity={setCamIntensity}
                      dubbingLang={dubbingLang}
                      onSetDubbingLang={setDubbingLang}
                      subtitleStyle={subtitleStyle}
                      onSetSubtitleStyle={setSubtitleStyle}
                      colorGrading={colorGrading}
                      onSetColorGrading={setColorGrading}
                      colorGradingEnabled={colorGradingEnabled}
                      onSetColorGradingEnabled={setColorGradingEnabled}
                      autoCutEnabled={autoCutEnabled}
                      onSetAutoCutEnabled={setAutoCutEnabled}
                      autoCutPreset={autoCutPreset}
                      onSetAutoCutPreset={setAutoCutPreset}
                      dubbingConfig={dubbingConfig}
                      onSetDubbingConfig={setDubbingConfig}
                      viralConfig={viralConfig}
                      onSetViralConfig={setViralConfig}
                      ...
                      useMuseTalk={useMuseTalk}
                      onSetUseMuseTalk={setUseMuseTalk}
                    />
                  </aside>
                )}
                ...
                >
                  ...
                  <StudioPanel
                    ...
                    onSelectScene={(s: Scene) => setEditingImageScene(s)}
              ...
              {editingImageScene && (
                <PhotoEditor
                  imageUrl={
                    ...
                    `data:image/svg+xml,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"><rect fill="#131a2c" width="400" height="400"/><text x="200" y="200" text-anchor="middle" fill="#8e9bb4" font-family="sans-serif" font-size="14">Sahne Görseli Yok</text></svg>')}`
                  }
                  onClose={() => setEditingImageScene(null)}
                  onSave={async (url: string) => {
                    const updated = scenes.map((s) =>
                      s.id === editingImageScene.id ? { ...s, image_path: url } : s,
                    );
