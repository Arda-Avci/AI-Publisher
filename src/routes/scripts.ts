...
scriptsRouter.post('/scripts/generate', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const status = error.message.includes('No characters') || error.message.includes('not found') ? 400 : 500;
    res.status(status).json({ error: error.message || 'Script oluşturulamadı.' });
  }
});

scriptsRouter.get('/:showId/scripts', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
  const userId = req.session.userId!;
...
scriptsRouter.get('/scripts/:scriptId', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
...
scriptsRouter.put('/scripts/:scriptId', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
  const { scriptId } = req.params;
...
scriptsRouter.delete('/scripts/:scriptId', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
  const { scriptId } = req.params;
...
scriptsRouter.put('/scripts/:scriptId/segments/:segmentId', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
...
scriptsRouter.post('/scripts/:scriptId/regenerate/:segmentId', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
  const { scriptId, segmentId } = req.params;
  ...
  try {
    ...
    const status = error.message.includes('not found') || error.message.includes('not belong') ? 400 : 500;
    res.status(status).json({ error: error.message || 'Segment yeniden oluşturulamadı.' });
  }
});

scriptsRouter.post('/scripts/:scriptId/produce', requireAuth, mediumLimiter, async (req: Request, res: Response) => {
  const userId = req.session.userId!;
  const { scriptId } = req.params;
