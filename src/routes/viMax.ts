...
import { extractCharacters, generateAvatarImages, saveCharacterImages } from '../services/autoCameo.js';
...
export function registerViMaxRoutes(app: Application): void {
  app.post('/api/v1/vimax/pipeline', heavyLimiter, requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/v1/vimax/auto-cameo', mediumLimiter, requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/v1/vimax/validate-consistency', mediumLimiter, requireAuth, async (req: Request, res: Response) => {
    try {
      const { jobId } = req.body;
      if (!jobId) {
        return res.status(400).json({ success: false, error: 'Job ID gerekli' });
  ...
  app.post('/api/v1/vimax/quality-inspect', mediumLimiter, requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/v1/vimax/rag-script', heavyLimiter, requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      await db.run(
        `UPDATE video_jobs SET scene_prompts = ?, total_scenes = ? WHERE id = ?`,
        [JSON.stringify(script.scenes), script.scenes.length, jobId]
      );
