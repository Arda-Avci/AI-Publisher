...
import { applySplitScreen, generateSplitScreenPreview, SplitLayout } from '../services/splitScreen.js';
import { Logger } from '../lib/logger.js';

export const splitRouter = Router();

splitRouter.post('/split/preview', mediumLimiter, requireAuth, async (req: Request, res: Response) => {
  try {
    const { primaryVideo, secondaryVideo, layout, position } = req.body;
    if (!primaryVideo || !secondaryVideo) {
      return res.status(400).json({ success: false, error: 'primaryVideo ve secondaryVideo gerekli' });
    }
...
splitRouter.post('/split/apply', mediumLimiter, requireAuth, async (req: Request, res: Response) => {
  try {
