...
nicheRouter.post('/niche/analyze', mediumLimiter, requireAuth, async (req: Request, res: Response) => {
  try {
...
nicheRouter.post('/niche/enhance-prompt', mediumLimiter, requireAuth, async (req: Request, res: Response) => {
  try {
    const { profile, platform, originalPrompt } = req.body;
    if (!profile || !platform || !originalPrompt) {
      return res.status(400).json({ success: false, error: 'profile, platform, originalPrompt gerekli' });
    }
