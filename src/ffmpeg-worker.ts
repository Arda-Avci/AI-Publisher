...
function getFfmpegMatrixPosition(position: string): string {
  switch (position) {
    ...
    case 'top_center':    return 'x=(w-tw)/2:y=40';
    case 'top_right':     return 'x=w-tw-40:y=40';
    case 'middle_left':   return 'x=40:y=(h-th)/2';
    case 'center':        return 'x=(w-tw)/2:y=(h-th)/2';
    case 'middle_right':  return 'x=w-tw-40:y=(h-th)/2';
    case 'bottom_left':   return 'x=40:y=h-th-40';
    case 'bottom_center': return 'x=(w-tw)/2:y=h-th-60'; // Klasik Shorts alani
    case 'bottom_right':  return 'x=w-tw-40:y=h-th-40';
    default:              return 'x=(w-tw)/2:y=h-th-100';
  }
}
...
async function startComposition() {
  try {
    ...
    const ffmpegCmd = `ffmpeg -y -i "${videoPath}" -i "${tempLogoPath}" -filter_complex ` +
    ...
    const process = exec(ffmpegCmd, (error) => {
      ...
      if (error) {
        parentPort?.postMessage({ status: 'error', error: 'GPU Hatasi. CPU Fallback tetikleniyor.' });
      } else {
    ...
    setTimeout(() => {
      ...
      if (fs.existsSync(tempLogoPath)) fs.unlinkSync(tempLogoPath);
      parentPort?.postMessage({ status: 'timeout_fallback', message: 'FFmpeg asili kalma korumasi tetiklendi.' });
    }, 30000);
