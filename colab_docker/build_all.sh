...
if [ -f "Dockerfile.base" ]; then
  ...
  DOCKER_BUILDKIT=1 docker build -t ai-publisher-base:latest -f Dockerfile.base .
...
for i in "${!MODELS[@]}"; do
  ...
  DOCKER_BUILDKIT=1 docker build -t "ai-publisher-$MODEL:latest" -f "$MODEL/Dockerfile" "$MODEL/"
